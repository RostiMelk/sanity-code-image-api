...
const hast_js_1 = require("../utils/hast.js");
const react_2 = require("react");
const default_json_1 = __importDefault(require("@sreetamdas/karma/themes/default.json"));
const SanityLogo_js_1 = require("../components/SanityLogo.js");
...
async function loadFonts() {
    if (!cachedFonts.jetBrains) {
        cachedFonts.jetBrains = await promises_1.default.readFile(node_path_1.default.join(process.cwd(), "fonts/jetbrains.ttf"));
    }
    if (!cachedFonts.inter) {
        cachedFonts.inter = await promises_1.default.readFile(node_path_1.default.join(process.cwd(), "fonts/inter.ttf"));
    }
    return {
    ...
    try {
        ...
        const code = shiki.codeToHtml(value, {
            ...
            transformers: [
                {
                    code(node) {
                        (0, hast_js_1.addTwToHast)(node, "flex-col w-full bg-[#1b1d27]");
                    },
                    line(node, line) {
                        (0, hast_js_1.addTwToHast)(node, "items-center pl-4 pr-12");
                        ...
                        if (highlightLines?.includes(line)) {
                            (0, hast_js_1.addTwToHast)(node, "bg-[#0d0e12]");
                        }
                    },
                    span(node) {
                        (0, hast_js_1.addTwToHast)(node, "shrink-0 leading-6");
                    },
                },
            ],
        });
        ...
        return new og_1.ImageResponse((react_1.default.createElement("main", { tw: `text-[${FONT_SIZE_PX}] flex w-full h-full bg-[#1b1d27] p-16` },
            react_1.default.createElement("figure", { tw: "flex-col shadow-2xl border-2 border-[#252837] rounded-3xl pb-10" },
                react_1.default.createElement("header", { tw: "flex items-center p-8 pl-9 pb-6" },
                    react_1.default.createElement(SanityLogo_js_1.SanityLogo, { tw: "w-15 h-15 mr-8 shadow-lg shadow-red-500/50 rounded-xl" }),
