...
async function loadFonts() {
    if (!cachedFonts.jetBrains) {
        cachedFonts.jetBrains = await fs.readFile(path.join(process.cwd(), "fonts/jetbrains.ttf"));
    }
    if (!cachedFonts.inter) {
        cachedFonts.inter = await fs.readFile(path.join(process.cwd(), "fonts/inter.ttf"));
    }
    return {
...
export async function createImage(props) {
    ...
    try {
        ...
        const cleanedParsedCode = cloneElement(parsedCode, parsedCode.props, cloneElement(parsedCode.props.children, parsedCode.props.children.props, cleanedChildren));
        const codeWithStyle = cloneElement(cleanedParsedCode, {
            tw: "m-0",
        });
        return new ImageResponse((React.createElement("main", { tw: `text-[${FONT_SIZE_PX}] flex w-full h-full bg-[#1b1d27] p-16` },
            React.createElement("figure", { tw: "flex-col shadow-2xl border-2 border-[#252837] rounded-3xl pb-10" },
                React.createElement("header", { tw: "flex items-center p-8 pl-9 pb-6" },
                    React.createElement(SanityLogo, { tw: "w-15 h-15 mr-8 shadow-lg shadow-red-500/50 rounded-xl" }),
                    React.createElement("span", { tw: "text-[#f6f6f8] text-2xl", style: { fontFamily: "inter" } }, fileName)),
